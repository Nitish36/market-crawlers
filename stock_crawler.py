...
def crawler():
    ...
    df.to_csv('datasets\\csv\\nifty50_stocks.csv', index=False)
    df.to_json('datasets\\json\\nifty50_stocks.json', orient="records")
    return df

def write_to_mongo():
    data_json = generate()
    collection = stock_db.stock
    stock_insert = collection.insert_one(data_json).inserted_id
